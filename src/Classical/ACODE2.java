...
/**
 * This simple recursive solution is taking to much time!
 * 
 * @author sagarjauhari@gmail.com
 * 
 */
class ACODE2 {
    String       PROB_NAME  = "ACODE";
    ...
    public static void main(String[] args) {
        ACODE2 program = new ACODE2(args);
        program.begin();
        program.end();
    }
    ...
    public void begin(){
        ...
        while(!str.equals("0")){
            int len = str.length();
            o_writ.printLine(decodings(str,len,0));
        
            str = i_scan.nextLine();
        }
    }
    
    int decodings(String str, int len, int low){
        if(low==len) return 1;
        if(low+1<len &&
        Integer.parseInt(str.substring(low,low+2))<=26){
            return (decodings(str,len,low+1) + decodings(str,len,low+2));
        }else{
            return (decodings(str,len,low+1));
        }
    }

    public ACODE2(String[] args) {
        if (args.length > 0 && args[0].equals("Test")) {
            i_scan = new InputScanner(new File(
                    "/home/sagar/code_arena/Eclipse Workspace/Spoj/inputs/"
                            + PROB_NAME + "/"+TEST_NAME));
        } else {
