...
class SCPC11D {
	...
	public static void main(String[] args) {
		SCPC11D program = new SCPC11D(args);
		program.begin();
		program.end();
	}
	...
	public void begin() {
		...
		int c = i_scan.nextInt();
		while (a > 0 || b > 0 || c > 0) {
			if (a == 0 || b == 0 || c == 0 || 
					a + b <= c || a + c <= b || b + c <= a) {
				o_writ.printLine("wrong"); //triangle not possible
				a = i_scan.nextInt();
				b = i_scan.nextInt();
				c = i_scan.nextInt();
				continue;
			...
			if (Math.pow(max, 2) == Math.pow(min1, 2) + Math.pow(min2, 2)){
				o_writ.printLine("right");
			}else{
				o_writ.printLine("wrong");
			}
